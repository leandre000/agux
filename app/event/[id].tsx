...
import { allEventsMock } from '../events/user'; // Import the mock data for booked info
...
export default function EventDetailScreen() {
    ...
    const event: any = allEventsMock.find(e => e.id === id) || {
        ...
        async function geocodeVenue() {
            ...
            try {
                const response = await fetch(
                    `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(event.venue)}`
                );
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.imageContainer}>
                    <RNImage source={event.image} style={styles.image} resizeMode="cover" />
                </View>

                {/* Event Info */}
                <View style={styles.infoSection}>
                    <View style={styles.infoRow}><Text style={styles.infoLabel}>Organizer</Text><Text style={styles.infoValue}>{event.organizer}</Text></View>
                    <View style={styles.infoRow}><Text style={styles.infoLabel}>Date</Text><View style={styles.infoIconRow}><Calendar size={16} color={Colors.text} /><Text style={styles.infoValue}>{event.date}</Text></View></View>
                    <View style={styles.infoRow}><Text style={styles.infoLabel}>Venue</Text><View style={styles.infoIconRow}><MapPin size={16} color={Colors.text} /><Text style={styles.infoValue}>{event.venue}</Text></View></View>
                </View>
                ...
                <View style={styles.mapContainer}>
                    <TouchableOpacity activeOpacity={0.95} onPress={() => router.push(`/event/${id}/map`)}>
                        <MapView
                            style={styles.mapImg}
                            initialRegion={{
